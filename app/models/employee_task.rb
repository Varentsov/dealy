class EmployeeTask < ActiveRecord::Base
  ...
  scope :in_active, -> { where(state: [EmployeeTask.states[:active], EmployeeTask.states[:confirmation]]) }
  scope :active_and_finished_tasks, -> { where(state: [EmployeeTask.states[:active], EmployeeTask.states[:confirmation], EmployeeTask.states[:completed]]) }
